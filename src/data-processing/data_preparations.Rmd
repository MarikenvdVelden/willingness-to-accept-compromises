...
For the categorical variable `party id`, we use the median value. 
...
We recode the missing values of `trust` and `willingness to accept compromise` variables to the mean value of the respective variables. 
For the categorical variables `party id` and `political interest`, we use the median value. 
...
We recode the missing values of the variables `Trust` and `Willingness to Accept Comprimise`, to `3` and include the variables `missing_trust` and `missing_wtac` to the data, indicating whether the response on the covariate was missing (value of 1) or not (value of 0).
...
We recode the missing values of the variable `Income`, to `4` and include the variable `missing_education` to the data, indicating whether the response on the covariate was missing (value of 1) or not (value of 0).
...
d %>%
  ...
  group_by(country) %>%
  summarise(avg = mean(trust),
            std = sd(trust)) %>%
  ggplot(mapping = aes(x = country, y = avg, 
                       ymin = avg - (1.96 * std), 
                       ymax = avg + (1.96 * std))) +
  geom_col(alpha = .2, fill = "seagreen") +
  geom_errorbar(position = position_dodge(width=0.9), width = 0.2, colour = "darkgrey") +
  labs(x = "", y = "Strongly agree (1) - Strongly disagree (5)", title = "Most politicians are trustworthy") +
  theme_minimal() +
...
d %>%
  ...
  geom_col(alpha = .2, fill = "seagreen") +
  geom_errorbar(position = position_dodge(width=0.9), width = 0.2, colour = "darkgrey") +
  labs(x = "", y = "Strongly agree (1) - Strongly disagree (5)", title = "Compromise is selling out principles") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
```
...
d %>%
  select(country, Gender = gender, `Party ID` = pid, 
         `Political Interest` = political_interest) %>%
  pivot_longer(cols = Gender:`Political Interest`) %>%
  ...
  scale_fill_manual(values = c("seagreen", "seagreen","violet", "seagreen", "violet","violet")) +
...
d %>%
  select(country, Age = age, Income = income, 
         Education = education, 
  ...
  facet_wrap(.~ name, scales = "free") +
  geom_errorbar(position = position_dodge(width=0.9), width = 0.2, colour = "darkgrey") +
  labs(x = "", y = "Mean Values", title = "Continuous Control Variables") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x=element_text(color = "black", size=11, 
                                 angle=30, vjust=.8, hjust=0.8))
...
### Correlations Matrix
```{r, echo=F, message=T, warning=T, fig.width=8, fig.height=8}
d %>%
mutate(political_interest = replace_na(political_interest, "Not Interested"),
         gender = replace_na(gender, "Male")) %>%
  select(`Political Trust` = trust,
