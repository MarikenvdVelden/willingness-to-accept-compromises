covs <- d %>%
  mutate(treat = 4,
         treat = ifelse(outcome == "stalled" & compromise == "yes", 1, treat),
         treat = ifelse(outcome == "stalled" & compromise == "no", 2, treat),
         treat = ifelse(outcome == "negotiation" & compromise == "yes", 3, treat),
  ...
  dplyr::select(treat, D1:D3,D4, D7:PT1_2, PT3_1, PT3_2, pol_know:PT8, MC1:MC3, POST) %>%
  drop_na()
...
balanced <-bal.tab(treat ~ factor(D1) +  factor(D2) + factor(D3) + 
                    ...
                    factor(MC1) + factor(MC2) + factor(MC3) + POST,
                   data = covs,
...
df <- balanced %>%
  mutate(variable = c("Male", "Age: 18 - 29", "Age: 30 - 39", "Age: 40 - 49",
                      ...
                      "Ideology", "Manipulation Check: Steadfast", 
                      "Manipulation Check: Outcome", "Coalition Knowledge",
                      "Populist Attitudes")) %>%
  ...
  drop_na() %>%
  mutate(type = ifelse(difference <= -.05, "below",
                        ifelse(difference >= .05, "below", "above"))) %>%
