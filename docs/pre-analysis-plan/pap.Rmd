...
kbl(d[1:11,], booktabs =T, caption = "\\label{tab:post1}Survey Questions - PostTreatment Questions (1)") %>%
  kable_styling(latex_options = c("striped", "hold_position"),
                full_width = F, fixed_thead = T, position = "center") %>%
  column_spec(1, width = "3cm") %>%
  column_spec(2, width = "7cm") %>%
...
kbl(d[12:23,], booktabs =T, caption = "\\label{tab:post2}Survey Questions - PostTreatment Questions (2)") %>%
  kable_styling(latex_options = c("striped", "hold_position"),
                full_width = F, fixed_thead = T, position = "center") %>%
  column_spec(1, width = "3cm") %>%
  column_spec(2, width = "7cm") %>%
...
kbl(d[24:dim(d)[1],], booktabs =T, caption = "\\label{tab:post3}Survey Questions - PostTreatment Questions (3)") %>%
  kable_styling(latex_options = c("striped", "hold_position"),
                full_width = F, fixed_thead = T, position = "center") %>%
  column_spec(1, width = "3cm") %>%
  column_spec(2, width = "7cm") %>%
...
## Hypothesis 1
We test the _steadfast hypothesis_ using Equation \ref{eq:h1} and the code chunk below.
...
We test the _responsibility hypothesis_ using Equation \ref{eq:h2} and the code chunk below.
...
We test the _compromise hypothesis_ using Equation \ref{eq:h3} and the code chunk below.
The coefficient of $\beta_{1}$ denotes the difference in the evaluation of steadfast and compromising parties.
The coefficient of $\beta_{2}$ denotes the difference in the evaluation of parties negotiating for coalition government and those who stop talking to potential coalition partners.
...
We test the _principled hypothesis_ using Equation \ref{eq:h4} and the code chunk below.
The coefficient of $\beta_{1}$ denotes the difference in the evaluation of steadfast and compromising parties.
...
## Hypothesis 5
 ...
 We test the _distrust hypothesis_ using Equation \ref{eq:h5} and the code chunk below.
The coefficient of $\beta_{1}$ denotes the difference in the evaluation of steadfast and compromising parties.
...
$\hat{Y}_{r, i}$ in Equation \ref{eq:pooled} denotes  the  evaluation  of  a party of respondent $r$, during issue $i$.
...
\begin{equation}\label{eq:pooled}
    \hat{Y}_{r, i} =\beta_{0} + \beta_{1}Intersection_{r, i} + \beta_{2}Compromise + \beta_{3}Compromise * Intersection + \alpha_{i} + \varepsilon_{r, i}
\end{equation}
...
d <- tibble(` ` = c(1:64),
            `In-Party` = c(rep("CDU", 16), rep("die Grünen", 16),
                           rep("FDP", 16), rep("SPD", 16)),
...
All stimulus material is developed by the [Network Institute of the _Vrije Universiteit Amsterdam_](https://networkinstitute.org/) and can be accessed [here](#). 
